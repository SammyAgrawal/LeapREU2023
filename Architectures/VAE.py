...
class VariationalEncoder(torch.nn.Module):
    ...
    """
    def __init__(self, in_dims, hidden_dims=[256, 128, 64, 32, 16], latent_dims=3, layers=1, dropout=0):
        super().__init__()
        ...
        for i, layer_size in range(layers):
            self.linears += [torch.nn.Sequential(
    ...
    def forward(self, x, return_latent=False):
        #y = torch.cat([y, x], 1) # don't get this, but used to have to pass in y and x
        if (type(x) != torch.Tensor):
            x = torch.tensor(x, dtype=torch.float32, )
        y = torch.flatten(x.squeeze(), start_dim=1)
...
class Decoder(torch.nn.Module):
    ...
    def forward(self, z):
        # z = torch.cat([z, x], 1), used to pass in x as well, confused
        for linear in self.linears:
...
class VariationalAutoencoder(torch.nn.Module):
    def __init__(self, beta=0.01, data_dims=124, label_dims=128,
        ...
        self.encoder = VariationalEncoder(data_dims, hidden_dims, latent_dims, layers, dropout)
        self.decoder = Decoder(label_dims, hidden_dims, latent_dims, layers, dropout)
        self.beta = beta
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'

    def forward(self, x, return_latent=False, batch_norm=False):
        # Normalize
        if batch_norm:
    ...
    def trainer(self, data, epochs=20, save="models/vae.cp", plot=True, loss_type='mse',
        ...
        losses = []
        for epoch, (x,y) in progress(range(epochs), inner=data, text='Training',
            ...
            if loss_type == 'mse':
                # iid gaussians -> mse
                # y_hat = y_mean + self.encoder.N.sample([x.shape[0], self.label_dims]) * y_std
