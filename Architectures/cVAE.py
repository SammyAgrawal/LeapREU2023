...
class VariationalEncoder(torch.nn.Module):
    ...
    """
    def __init__(self, in_dims, hidden_dims=512, latent_dims=3, layers=1, dropout=0, device='cpu'):
        super().__init__()
        self.linears = []
...
class ConditionalVAE(torch.nn.Module):
    def __init__(self, beta=0.01, data_dims=124, label_dims=128,
                 latent_dims=3, hidden_dims=512, layers=2, dropout=0, device='cpu'):
        """
        Conditional VAE
        ...
        self.encoder = VariationalEncoder(label_dims + data_dims, hidden_dims, latent_dims, layers, dropout, device)
    ...
    def sample(self, x, random=True):
        ...
        if random:
            ...
            # Set to prior mean
            z = torch.zeros([x.shape[0], self.latent_dims]).to(self.device)
